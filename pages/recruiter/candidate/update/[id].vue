<script lang="ts">
    ...
    export default {
        ...
        data() {
            return {
                dataToPass: {
                    current: this.$t('update_framework'),
                    list: [this.$t('dashboard'), this.$t('framework'), this.$t('update_framework')],
                },
                kpiData: kpiData,
                testStore: reactive(useTestStore()),
                operationyear: [] as Array<any>,
                body: {
                    id: 0,
                    code: 'PS-P',
                    frameworkNameEn: '',
        ...
        mounted() {
            ...
            let framework = this.testStore.getFramework(+frameworkId);
            if (framework) {
                this.body.id = framework.id;
                this.body.code = framework.code;
                this.body.frameworkNameEn = framework.frameworkNameEn;
                this.body.frameworkNameKh = framework.frameworkNameKh;
                this.body.moto = framework.moto;
                this.body.kp = framework.kp;
                this.body.stategic_goal = framework.stategic_goal;
                this.body.approach = framework.approach;
                this.body.logicalFramework = framework.logicalFramework;
                this.operationyear[0] = framework.start_year;
                this.operationyear[1] = framework.end_year;
                this.body.line_ministry = framework.line_ministry;
                this.body.status = framework.status;
            }
        },
        methods: {
            onChangeYear() {
            ...
            submitUpdateForm() {
                ...
                if (validateMsg === 'ok') {
                    let body = this.body;
                    const respond = this.testStore.updateFramework(body.id, body);
                    if (respond === 'ok') {
        ...
        computed: {
            validateForm() {
                ...
                // if (!this.body.end_year) this.errors.end_year = this.t('validation.required');
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <el-checkbox-group v-model="body.logicalFramework" class="select-logical-container d-flex flex-wrap justify-content-between">
                                <div class="card" v-for="item in logical_frameworks" :key="item.id">
                                    <div class="card-body" style="border-radius: 8px; box-shadow: 2px 4px 16px rgba(0,0,0,.08);">
                        ...
                        <div class="col-12">
                            <div class="btn-section float-right">
                                ...
                                <el-button @click="submitUpdateForm" type="success" class="btn-create">{{ t('page_btn.request_btn') }}</el-button>
                            </div>
                        </div>
                    </div>
                </div>
